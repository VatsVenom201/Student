...
def HOME(request):
    ...
    attendance_present = Attendance_Report.objects.filter(student_id=student_obj).count()
    
    # Calculate total available attendance sessions for the student's subjects
    subjects = Subject.objects.filter(course=student_obj.course_id)
    ...
    if total_attendance == 0:  # Avoid division by zero
        ...
        attendance_string = str(int(percentage)) + "%"

    subject_count = Subject.objects.filter(course=student_obj.course_id).count() 
    apply_leave = Student_leave.objects.filter(student_id=student_obj).count()

    context = {
        'total_attendance': attendance_string,
